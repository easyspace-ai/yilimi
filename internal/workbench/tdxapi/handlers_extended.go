// Extended HTTP handlers ported from test/tdx-api/web/server_api_extended.go.
// Upstream 新增接口时：在此文件按相同模式加 (*Service) 方法，并在 service.go 的 mux() 中注册。
...
// 获取股票代码列表
func (s *Service) handleGetCodes(w http.ResponseWriter, r *http.Request) {
	...
	allCodes, err := s.getAllCodeModels()
	if err != nil {
...
// 批量获取行情
func (s *Service) handleBatchQuote(w http.ResponseWriter, r *http.Request) {
	...
	quotes, err := s.client.GetQuote(req.Codes...)
	if err != nil {
...
func (s *Service) handleGetKlineHistory(w http.ResponseWriter, r *http.Request) {
	...
	switch klineType {
	case "minute1":
		resp, err = s.client.GetKlineMinute(code, 0, limit)
	case "minute5":
		resp, err = s.client.GetKline5Minute(code, 0, limit)
	case "minute15":
		resp, err = s.client.GetKline15Minute(code, 0, limit)
	case "minute30":
		resp, err = s.client.GetKline30Minute(code, 0, limit)
	case "hour":
		resp, err = s.client.GetKlineHour(code, 0, limit)
	case "week":
		// 周K线使用前复权
...
// 获取指数数据
func (s *Service) handleGetIndex(w http.ResponseWriter, r *http.Request) {
	code := r.URL.Query().Get("code")
	klineType := r.URL.Query().Get("type")
	limitStr := r.URL.Query().Get("limit")
	...
	switch klineType {
	case "minute1":
		resp, err = s.client.GetIndex(protocol.TypeKlineMinute, code, 0, limit)
	case "minute5":
		resp, err = s.client.GetIndex(protocol.TypeKline5Minute, code, 0, limit)
	case "minute15":
		resp, err = s.client.GetIndex(protocol.TypeKline15Minute, code, 0, limit)
	case "minute30":
		resp, err = s.client.GetIndex(protocol.TypeKline30Minute, code, 0, limit)
	case "hour":
		resp, err = s.client.GetIndex(protocol.TypeKline60Minute, code, 0, limit)
	case "week":
		resp, err = s.client.GetIndexWeekAll(code)
		if resp != nil && len(resp.List) > int(limit) {
			resp.List = resp.List[:limit]
			resp.Count = limit
		}
	case "month":
		resp, err = s.client.GetIndexMonthAll(code)
		if resp != nil && len(resp.List) > int(limit) {
			resp.List = resp.List[:limit]
			resp.Count = limit
		}
	case "day":
		fallthrough
	default:
		resp, err = s.client.GetIndexDay(code, 0, limit)
	}
...
// 获取指数全部历史K线
func (s *Service) handleGetIndexAll(w http.ResponseWriter, r *http.Request) {
	...
	limit := s.parsePositiveInt(r.URL.Query().Get("limit"))
	list, err := s.fetchIndexAll(code, klineType)
	if err != nil {
...
// 获取市场统计
func (s *Service) handleGetMarketStats(w http.ResponseWriter, r *http.Request) {
	...
	for _, model := range allCodes {
		...
		case "sh":
			stats.SH.Total++
			s.classifyPrice(lastPrice, &stats.SH.Up, &stats.SH.Down, &stats.SH.Flat)
		case "sz":
			stats.SZ.Total++
			s.classifyPrice(lastPrice, &stats.SZ.Up, &stats.SZ.Down, &stats.SZ.Flat)
		case "bj":
			stats.BJ.Total++
			s.classifyPrice(lastPrice, &stats.BJ.Up, &stats.BJ.Down, &stats.BJ.Flat)
		}
	}
...
// 获取各交易所证券数量
func (s *Service) handleGetMarketCount(w http.ResponseWriter, r *http.Request) {
	...
	for _, ex := range exchanges {
		countResp, err := s.client.GetCount(ex)
		if err != nil {
...
// 获取全部股票代码列表
func (s *Service) handleGetStockCodes(w http.ResponseWriter, r *http.Request) {
...
// 获取全部ETF代码列表
func (s *Service) handleGetETFCodes(w http.ResponseWriter, r *http.Request) {
	if tdx.DefaultCodes == nil {
...
// 获取ETF列表
func (s *Service) handleGetETFList(w http.ResponseWriter, r *http.Request) {
...
// 获取历史分时成交（分页）
func (s *Service) handleGetTradeHistory(w http.ResponseWriter, r *http.Request) {
	code := strings.TrimSpace(r.URL.Query().Get("code"))
	...
	resp, err := s.client.GetHistoryMinuteTrade(date, code, uint16(start), uint16(count))
	if err != nil {
...
// 获取全天分时成交
func (s *Service) handleGetMinuteTradeAll(w http.ResponseWriter, r *http.Request) {
	code := strings.TrimSpace(r.URL.Query().Get("code"))
	date := strings.TrimSpace(r.URL.Query().Get("date"))
	...
	if date != "" {
		...
		resp, err = s.client.GetMinuteTradeAll(code)
	}
	if err != nil {
...
// 获取上市以来的全部历史分时成交
func (s *Service) handleGetTradeHistoryFull(w http.ResponseWriter, r *http.Request) {
	code := strings.TrimSpace(r.URL.Query().Get("code"))
	...
	includeToday := s.parseBool(strings.TrimSpace(r.URL.Query().Get("include_today")))

	if code == "" {
		errorResponse(w, "code 为必填参数")
		return
	}
	if s.manager == nil || s.manager.Workday == nil {
	...
	if startParam != "" {
		start, err = s.parseWorkdayDate(startParam)
		if err != nil {
	...
	if beforeParam != "" {
		end, err = s.parseWorkdayDate(beforeParam)
		if err != nil {
			errorResponse(w, "before 参数格式错误，应为 YYYYMMDD 或 YYYY-MM-DD")
			return
		}
	} else if endParam != "" {
		end, err = s.parseWorkdayDate(endParam)
		if err != nil {
	...
	if !start.After(historyEnd) {
		s.manager.Workday.Range(
			...
			func(t time.Time) bool {
				...
				resp, err := s.client.GetHistoryMinuteTradeDay(dateStr, code)
				if err != nil {
					lastErr = err
					return true
				}
...
// 获取股票全部历史K线（通达信）
func (s *Service) handleGetKlineAllTDX(w http.ResponseWriter, r *http.Request) {
	code := strings.TrimSpace(r.URL.Query().Get("code"))
	if code == "" {
		errorResponse(w, "股票代码不能为空")
		return
	...
	list, err := s.fetchStockKlineAllTDX(code, klineType)
	if err != nil {
		errorResponse(w, fmt.Sprintf("获取K线失败: %v", err))
		return
	}
	...
	s.respondKlineSuccess(w, "tdx", klineType, list)
}

// 获取股票全部历史K线（同花顺前复权）
func (s *Service) handleGetKlineAllTHS(w http.ResponseWriter, r *http.Request) {
	code := strings.TrimSpace(r.URL.Query().Get("code"))
	if code == "" {
		errorResponse(w, "股票代码不能为空")
		return
	...
	list, err := s.fetchStockKlineAllTHS(code, klineType)
	if err != nil {
	...
	s.respondKlineSuccess(w, "ths", klineType, list)
}

// 获取交易日信息
func (s *Service) handleGetWorkday(w http.ResponseWriter, r *http.Request) {
	if s.manager == nil || s.manager.Workday == nil {
		errorResponse(w, "交易日模块未初始化")
		return
	}
	...
	target := time.Now()
	if dateParam != "" {
		parsed, err := s.parseWorkdayDate(dateParam)
		if err != nil {
	...
	isWorkday := s.manager.Workday.Is(target)
	...
	response := map[string]interface{}{
		...
		"next":       s.collectNeighborWorkdays(target, count, 1),
		"previous":   s.collectNeighborWorkdays(target, count, -1),
	}
...
// 获取指定范围内的交易日列表
func (s *Service) handleGetWorkdayRange(w http.ResponseWriter, r *http.Request) {
	if s.manager == nil || s.manager.Workday == nil {
		errorResponse(w, "交易日模块未初始化")
		return
	}
	...
	startDate, err := s.parseWorkdayDate(startParam)
	if err != nil {
		errorResponse(w, "start 参数格式错误，应为 YYYYMMDD 或 YYYY-MM-DD")
		return
	}
	endDate, err := s.parseWorkdayDate(endParam)
	if err != nil {
	...
	s.manager.Workday.Range(startDate, endDate.AddDate(0, 0, 1), func(t time.Time) bool {
	...
	if len(list) == 0 {
		if err := s.manager.Workday.Update(); err == nil {
			s.manager.Workday.Range(startDate, endDate.AddDate(0, 0, 1), func(t time.Time) bool {
				list = append(list, map[string]string{
					"iso":     t.Format("2006-01-02"),
					"numeric": t.Format("20060102"),
...
// 获取服务器状态
func (s *Service) handleGetServerStatus(w http.ResponseWriter, r *http.Request) {
...
// 基于K线的收益率计算
func (s *Service) handleGetIncome(w http.ResponseWriter, r *http.Request) {
	code := strings.TrimSpace(r.URL.Query().Get("code"))
	startParam := strings.TrimSpace(r.URL.Query().Get("start_date"))
	...
	startDate, err := s.parseFullDate(startParam)
	if err != nil {
		errorResponse(w, "start_date 格式错误，应为 YYYYMMDD 或 YYYY-MM-DD")
		return
	}

	dayOffsets := s.parseDaysParam(daysParam)
	if len(dayOffsets) == 0 {
	...
	klines := s.buildExtendKlines(code, resp.List)
...
func (s *Service) getAllCodeModels() ([]*tdx.CodeModel, error) {
	if tdx.DefaultCodes != nil {
	...
	for _, ex := range []protocol.Exchange{protocol.ExchangeSH, protocol.ExchangeSZ, protocol.ExchangeBJ} {
		resp, err := s.client.GetCodeAll(ex)
		if err != nil || resp == nil {
			if err != nil {
...
func (s *Service) classifyPrice(price float64, up, down, flat *int) {
	switch {
	case price > 0:
		*up = *up + 1
	case price < 0:
...
func (s *Service) parseWorkdayDate(value string) (time.Time, error) {
...
func (s *Service) collectNeighborWorkdays(base time.Time, count int, step int) []map[string]string {
	...
	for len(result) < count && attempts < maxAttempts {
		...
		if s.manager.Workday.Is(cursor) {
...
func (s *Service) parsePositiveInt(value string) int {
	if value == "" {
		return 0
	}
...
func (s *Service) parseFullDate(value string) (time.Time, error) {
	t, err := s.parseWorkdayDate(value)
	if err != nil {
...
func (s *Service) parseDaysParam(value string) []int {
	if value == "" {
		return nil
	}
...
func (s *Service) buildExtendKlines(code string, list []*protocol.Kline) extend.Klines {
...
func (s *Service) parseBool(value string) bool {
	if value == "" {
		return false
	}
...
func (s *Service) fetchStockKlineAllTDX(code, klineType string) ([]*protocol.Kline, error) {
	...
	case "hour":
		...
		return resp.List, nil
	case "day":
		resp, err := s.client.GetKlineDayAll(code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "week":
		resp, err := s.client.GetKlineWeekAll(code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "month":
		resp, err := s.client.GetKlineMonthAll(code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "quarter":
		resp, err := s.client.GetKlineQuarterAll(code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "year":
		resp, err := s.client.GetKlineYearAll(code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
...
func (s *Service) fetchStockKlineAllTHS(code, klineType string) ([]*protocol.Kline, error) {
	resp, err := s.getQfqKlineDay(code)
	if err != nil {
		return nil, err
	}
	...
	case "", "day":
		return resp.List, nil
	case "week":
		return s.convertToWeekKline(resp).List, nil
	case "month":
		return s.convertToMonthKline(resp).List, nil
	default:
		return nil, fmt.Errorf("同花顺接口暂仅支持 type=day/week/month")
	}
}

func (s *Service) respondKlineSuccess(w http.ResponseWriter, source, klineType string, list []*protocol.Kline) {
...
func (s *Service) fetchIndexAll(code, klineType string) ([]*protocol.Kline, error) {
	switch strings.ToLower(klineType) {
	case "minute1":
		resp, err := s.client.GetIndexAll(protocol.TypeKlineMinute, code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "minute5":
		resp, err := s.client.GetIndexAll(protocol.TypeKline5Minute, code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "minute15":
		resp, err := s.client.GetIndexAll(protocol.TypeKline15Minute, code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "minute30":
		resp, err := s.client.GetIndexAll(protocol.TypeKline30Minute, code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "hour":
		resp, err := s.client.GetIndexAll(protocol.TypeKline60Minute, code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	...
	case "month":
		...
		return resp.List, nil
	case "quarter":
		resp, err := s.client.GetIndexQuarterAll(code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "year":
		resp, err := s.client.GetIndexYearAll(code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
	case "day":
		fallthrough
	default:
		resp, err := s.client.GetIndexDayAll(code)
		if err != nil {
			return nil, err
		}
		return resp.List, nil
